...
exports.parseHourlyKpiCsv = functions.https.onCall(async (data, context) => {
    ...
    if (records[0].length !== 85) {
        throw new functions.https.HttpsError('invalid-argument', `Invalid column count: ${records[0].length} (expected 85)`);
    }
    // 7. 時間帯定義（14時間帯）
    const HOUR_LABELS = [
        'early',
...
exports.commitCsvImport = functions.https.onCall(async (data, context) => {
    if (!context.auth || !['executive', 'manager'].includes(context.auth.token.role)) {
