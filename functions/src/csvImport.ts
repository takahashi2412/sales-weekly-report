...
export const parseHourlyKpiCsv = functions.https.onCall(async (data, context) => {
  ...
  let productId: string, sy: string, sm: string, sd: string;
  if (matchNew) {
    ...
    throw new functions.https.HttpsError('invalid-argument',
      'Invalid filename. Expected: 時間別_{productId}_YYYYMMDD.csv');
  }
...
export const commitCsvImport = functions.https.onCall(async (data, context) => {
  ...
  for (const member of members) {
    ...
    for (const key of KPI_KEYS) {
      calculatedTotals[key] = (member.hourlyData || []).reduce((s: number, h: any) => s + (Number(h[key]) || 0), 0);
    }
  ...
  const summaryTotals: any = {};
  for (const key of KPI_KEYS) {
    summaryTotals[key] = (summary.hourlyData || []).reduce((s: number, h: any) => s + (Number(h[key]) || 0), 0);
  }
